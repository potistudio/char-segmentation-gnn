...
ja-lyric 4,000サンプル / 10エポック、別シードの評価セット、各モデル自分の最良閾値:

| `--text-ratio` | 汎用 | 混同 | 混同の過結合 |
| --- | --- | --- | --- |
| なし | **0.8845** | 0.7290 | 0.2938 |
| 0.05 | 0.8765 | 0.7123 | 0.2877 |
| 0.15 | 0.8620 | 0.7551 | 0.0809 |
| **0.30** | 0.8650 | **0.7865** | **0.0672** |

- 混同セットの誤結合が **0.2938 → 0.0672 (−77%)**
- 汎用のコストは −2pt あたりで頭打ちになるが、混同の利得は伸び続ける。
  0.30 は 0.15 より**両軸で良い**
- 0.05 は混同でも悪化した。少量では識別を学べないまま動作点だけ動くように見える
  (1シードなのでノイズの可能性もある)
- **混同セットでは同じ誤結合率に揃えても mix が優位**。誤結合 0.20 の点で
  過分割 0.0885 対 0.1172、一致率 0.7394 対 0.6940。単なる交換ではない
