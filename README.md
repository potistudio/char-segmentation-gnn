...
1文字分にも足りないので、あるエッジは隣のグリフを一度も見ないまま判断している。
「ll」が2文字か1つの幅広グリフかを分けるのは文字ピッチだが、その距離では観測できない。
グラフ全体をプールすれば、リダクション1回のコストでそれが分類器の手に入る。

`--no-global-context` で外せる(アブレーション用)。ja 6,000 サンプルでの実測:

| 構成 | seed 5 | seed 99 |
| --- | --- | --- |
| readout なし | 0.7692 | 0.7300 |
| **readout あり** | **0.7875** | **0.7525** |
