...
| オプション | 既定 | 意味 |
| --- | --- | --- |
| `--tracking-min` | −0.18 | 最も詰めた側。**裾の端で、典型値ではない** |
| `--tracking-max` | +0.04 | 最も緩めた側。0 を越える必要がある(後述) |
| `--tracking-skew` | 1.8 | 分布の形。1 で一様、大きいほど `--tracking-max` 寄り |
| `--baseline-jitter` | 0.02 | 1グリフあたりの縦揺れ上限(em)。0 で無効 |
| `--aspect-x-min` / `--max` | 0.8 / 1.25 | 行全体の横倍率(condensed / extended) |
| `--point-noise` | 0.004 | 全アウトライン点に加えるジッタ上限(em) |

既定の tracking 分布は**中央値 −0.02 em**(通常の組版よりわずかに詰め)で、
約3割が 0 より緩く、約2〜3割が −0.08 em より詰まる。実測値は
`cargo test -p dataset-gen` が範囲ごとに検証している。

- **トラッキングは1サンプルにつき1回**引かれる。行内でピッチが規則的に保たれ、
  それがモデルが必要とする手がかりそのものになる
- 一様分布ではなく偏らせている理由: 通常の組版は 0〜+0.05 em、
  詰めた見出しで −0.02〜−0.05 em、ロゴの強い詰めで −0.10 em 程度。
  一様 `Uniform(−0.18, +0.02)` だとコーパスの2/3が実際より詰まった状態になり、
  かつ**緩い行をモデルがほとんど見ない**
- `--baseline-jitter` の既定を 0.06 → 0.02 に下げている。正しく組まれた
  テキストのベースラインは揺れず、しかもベースライン整合は
  グルーピングの実信号(「こ」の上下2画か「一 一」の2文字か)。
  グリフごとに独立して揺らすと、頑健性を足すより手がかりを隠すほうに働く
