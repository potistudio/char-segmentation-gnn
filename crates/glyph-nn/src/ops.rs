...
/// Rows per task in [`linear`]. `matrixmultiply::sgemm` is single threaded, so
/// the split has to happen here; below this the rayon hand-off costs more than
/// the multiply, which matters because the contour layers call this with 38
/// rows while the classifier calls it with thousands.
const GEMM_ROW_BLOCK: usize = 128;
...
pub fn linear(x: &[f32], m: usize, w: View<'_>, b: View<'_>, out: &mut [f32]) {
    ...
    out.par_chunks_mut(GEMM_ROW_BLOCK * n)
        .zip(x.par_chunks(GEMM_ROW_BLOCK * k))
